...
export default function DepartmentFilter({
  ...
  return (
    <div className="relative p-3 sm:p-4 border-b-0">
      <button
        className="w-full flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 border rounded-xl transition-all duration-200 shadow-sm hover:shadow-md group"
        style={{ 
          backgroundColor: 'var(--card-bg)', 
          borderColor: 'var(--border-color)'
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.backgroundColor = isDark ? 'rgba(139, 92, 246, 0.1)' : 'rgba(245, 243, 255, 1)';
      ...
      >
        <div className="flex items-center gap-2 sm:gap-3 flex-1">
          ...
          <div className="text-left flex-1">
            <p className="text-[10px] sm:text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>Department</p>
        ...
        <ChevronDown 
          size={16} 
          className={`transition-transform duration-200 sm:w-5 sm:h-5 ${
            isOpen ? 'rotate-180' : ''
          }`}
      ...
      {isOpen && (
        <>
          ...
          {/* Dropdown Menu */}
          <div className="absolute left-3 right-3 sm:left-4 sm:right-4 top-full mt-2 border rounded-xl shadow-xl z-50 overflow-hidden animate-slide-in max-h-64 overflow-y-auto custom-scrollbar" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
