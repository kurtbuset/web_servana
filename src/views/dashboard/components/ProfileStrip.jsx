...
export default function ProfileStrip() {
  ...
  return (
    <div className="profile-strip">
      <div className="profile-avatar-lg">{getInitials(fullName)}</div>
      <div>
        <div className="profile-name">Hi, {fullName}</div>
        <div className="profile-email">{email}</div>
        <div className="profile-badges">
          <span className="badge badge-purple">{roleName}</span>
          {departments.length > 0 ? (
            ...
            <span className="badge badge-cyan">No Department</span>
          )}
        </div>
      </div>
    </div>
