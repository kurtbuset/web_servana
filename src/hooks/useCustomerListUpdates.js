...
export const useCustomerListUpdates = (
  ...
  const handleMoveToTopOrNewAssignment = useCallback(
    (customer, updateType) => {
      setDepartmentCustomers((prevDeptCustomers) => {
        ...
        const chat_type = customer.status === "queued" ? "queued" : "active";
   ...
   */
  const handleNewQueuedChat = useCallback(
    (customer) => {
      setDepartmentCustomers((prevDeptCustomers) => {
        const updatedDeptCustomers = { ...prevDeptCustomers };
        ...
        if (!exists) {
          ...
          console.log(
            `✅ New queued chat arrived: ${customer.name} (${customer.chat_group_id})`,
          );
        }
  ...
  const handleChatAccepted = useCallback(
    (customer) => {
      setDepartmentCustomers((prevDeptCustomers) => {
        ...
        console.log(
          `✅ Chat accepted and moved to active: ${customer.name} (${customer.chat_group_id})`,
        );
   ...
   */
  const handleRemoveChatGroup = useCallback(
    (chatGroupId, acceptedBy) => {
      const userId = getUserId();

      console.log(`🔍 handleRemoveChatGroup called:`, {
        chatGroupId,
        acceptedBy,
        currentUserId: userId,
        shouldRemove: acceptedBy !== userId,
      });
      ...
      if (acceptedBy !== userId) {
          ...
          Object.keys(updatedDeptCustomers).forEach((dept) => {
            ...
            updatedDeptCustomers[dept] = updatedDeptCustomers[dept].filter(
              (existingCustomer) => {
                ...
                const match =
                  String(existingCustomer.chat_group_id) ===
                  String(chatGroupId);
                if (match) {
                  console.log(
                    `🎯 Found matching chat to remove:`,
            ...
            if (beforeCount !== afterCount) {
              console.log(
                `🗑️ Removed chat ${chatGroupId} from department ${dept} (${beforeCount} -> ${afterCount})`,
              );
            }
          });

          console.log(
            `✅ Chat ${chatGroupId} removed from customer list (accepted by agent ${acceptedBy})`,
          );

          return updatedDeptCustomers;
        });

        // Also clear selected customer if it was the one that got accepted
        setSelectedCustomer((prev) => {
          if (prev && String(prev.chat_group_id) === String(chatGroupId)) {
            console.log(
              `✅ Clearing selected customer ${chatGroupId} (accepted by another agent)`,
            );
            return null;
          }
          return prev;
        ...
        console.log(`⏭️ Skipping removal - current user accepted the chat`);
      }
    },
   ...
   */
  const handleCustomerListUpdate = useCallback(
    (updateData) => {
      console.log("📨 customerListUpdate received:", {
